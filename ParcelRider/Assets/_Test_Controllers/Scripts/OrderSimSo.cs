...
public class OrderSimSo : ScriptableObject
{
    ...
    public (string order, int state) DoStateUpdate(DeliverOrderModel order, int stateId) => _field.DoState_Update(order, stateId);
    ...
    [Serializable] private class OrderField
    {
        ...
        public (bool isSuccess, string order) OrderAssignedResponse(DeliverOrderModel model)
        {
            ...
            var state = DoStateMap.GetState(100);
            o.RiderId = order.RiderId;
            o.Rider = order.Rider;
            o.Status = assigned;
            o.SubState = state.StateId;
            var histories = o.StateHistory.ToList();
            histories.Add(new Info_StageSegment{SubState =  100, Timestamp = DateTime.Now});
            o.StateHistory = histories.ToArray();
            var bag = DataBag.Serialize(o);
            return (true, bag);
        }

        public (string order, int state) DoState_Update(DeliverOrderModel order, int stateId)
        {
            var o = GetOrder(order.Id);
