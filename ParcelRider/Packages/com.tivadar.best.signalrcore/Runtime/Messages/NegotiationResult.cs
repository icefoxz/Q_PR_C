...
{
    /// <summary>
    /// Represents a transport with its supported transfer formats, used in the <see cref="NegotiationResult"/>.
    /// </summary>
    ...
    public sealed class NegotiationResult
    {
        ...
        /// <summary>
        /// The <see cref="HTTP.HTTPResponse"/> instance the negotiation-response received with.
        /// </summary>
        public HTTP.HTTPResponse NegotiationResponse { get; internal set; }

        internal static NegotiationResult Parse(HTTP.HTTPResponse resp, out string error, HubConnection hub)
        {
            error = null;

            Dictionary<string, object> response = Best.HTTP.JSON.Json.Decode(resp.DataAsText) as Dictionary<string, object>;
            if (response == null)
            {
