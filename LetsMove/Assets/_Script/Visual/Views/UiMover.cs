...
{
    [SerializeField] private float _shake = 0.1f;
    ...
    private void ForceStopCoroutine()
    {
        StopAllCoroutines();
        _isMoving = false;
    }
    ...
    //抖动(0.1秒)后才判断需不需要移动
    private IEnumerator Shake(Vector2 movePosition, float duration, Action<bool> callbackAction)
    {
    ...
    private IEnumerator MoveRectTransform(Vector2 pos, float duration, Action callbackAction)
    {
        _isMoving = true;
        var time = 0f;
